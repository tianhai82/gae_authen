...
func TestGetJwtForUser(t *testing.T) {
	...
	defer c.Close()
	tokenString, err := getJwtForUser("yinloo")
	if err != nil {
	...
	token, err = ParseToken(strings.Replace(tokenString, "e", "E", 1))
	if err == nil {
	...
	token2, err2 := ParseToken(newTokenString)
	if err2 == nil {
	...
	newTokenString, signErr = newToken.SignedString(privateKey)
